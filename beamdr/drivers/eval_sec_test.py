...
def generate_new_ann(args):
    ...
    with dev_query_cache as emb:
        dev_query_embedding, dev_query_embedding2id = StreamInferenceDoc(args, model, GetProcessingFn(args, query=True), "dev-sec_"+ str(latest_step_num)+"_", emb, is_query_inference = True, load_cache = args.load_cache)
    #exit()
...
def validate(args, closest_docs, dev_scores, query_embedding2id, passage_embedding2id):
    ...
    first_hop_ets = pickle.load(open(args.data_dir + '/dev_first_hop_pred.pkl', 'rb'))
    pred_dict = dict()
    for query_idx in range(closest_docs.shape[0]): 
        ...
        pre_score = first_hop_ets[qid]['score'][first_hop_ets[qid]['pred'].index(pre_et)]
        ...
        for i in range(len(dev_scores[query_idx])):
            if int(passage_embedding2id[all_pred[i]]) in offset2pid:
                pred_dict[qid]['chain'].append(pre_et + '#######' + normalize(idx2title[offset2pid[int(passage_embedding2id[all_pred[i]])]]))
    ...
    for data in dataset:
        ...
        if qid in pred_dict:
            ...
            idxs = sorted(range(len(doc_scores)), key=lambda k: doc_scores[k], reverse=True)
            for idx in idxs[:250]:
                all_pairs.append(pred_dict[qid]['chain'][idx])
        sec_hop_pred[qid] = all_pairs    

    pickle.dump(sec_hop_pred, open(args.data_dir + '/dev_sec_hop_pred_top250.pkl', 'wb'))
