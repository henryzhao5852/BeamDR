...
def write_qas_query(args, pid2offset, qas_file, out_query_file, out_ann_file):
    ...
    with open(qas_path, "r", encoding="utf-8") as f, open(out_query_path, "wb") as out_query, open(out_ann_file, "w", encoding='utf-8') as out_ann:
        ...
        for sample in data:
            qqid = sample['qid']
            first_pos_pid = [str(pid2offset[int(pos_ctx)]) for pos_ctx in sample['answers']]
            question = normalize_question(sample['question'])
            out_ann.write("{}\t{}\n".format(qqid, ','.join(first_pos_pid)) )
...
def preprocess(args):
    ...
    write_qas_query(args, pid2offset, "hotpot_train_first_hop.json", "train-query", "train-ann")
    write_qas_query(args, pid2offset, "hotpot_dev_first_hop.json", "dev-query", "dev-ann")
    write_qas_query(args, pid2offset, "hotpot_train_sec_hop.json", "train-sec-query", "train-sec-ann")
    write_qas_query(args, pid2offset, "hotpot_dev_sec_hop.json", "dev-sec-query", "dev-sec-ann")
