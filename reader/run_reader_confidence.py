...
def main():
    ...
    parser.add_argument("--output_load_dir", default=None, type=str,
                        help="The output directory where the model checkpoints and predictions will be written.")
    # Other parameters
    ...
    parser.add_argument('--save_gran',
                        type=str, default='40, 10',
                        help='"10,5" means saving a checkpoint every 1/10 of the total updates, but start saving from the 5th attempt')
    args = parser.parse_args()
    print(args)
    lr = str(args.learning_rate)
    b_sz = str(args.train_batch_size)
    ep = str(args.num_train_epochs)
    lam = str(args.lambda_scale)
    writer = SummaryWriter(comment="LARGE_LR_" + lr +"_BATCH_" + b_sz + '_' + 'EPOCH_' + ep + '_LAMBDA_' + lam)
    ...
    #num_train_optimization_steps = 30000
    ...
    if args.bert_model != 'bert-large-uncased-whole-word-masking':
        ...
        args.output_load_dir,  num_labels=4, no_masking=args.no_masking)
        tokenizer = BertTokenizer.from_pretrained(
        args.output_load_dir, do_lower_case=args.do_lower_case)
    else:
    ...
    if args.fp16:
        ...
        num_train_optimization_steps = 30000
        scheduler = WarmupLinearSchedule(optimizer, warmup_steps=2000, t_total=num_train_optimization_steps)
    ...
    if args.do_train:
        ...
        try:
            ...
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
                ...
                with open(cached_train_features_file, "wb") as wri:
                    pickle.dump(train_features, wri)
        #exit()
        ...
        all_start_positions = torch.tensor(
            [f.start_position for f in eval_features], dtype=torch.long)
        all_end_positions = torch.tensor(
            [f.end_position for f in eval_features], dtype=torch.long)
        all_switches = torch.tensor(
            [f.switch for f in eval_features], dtype=torch.long)
        all_span_mask = torch.tensor(
            [f.switch ==0  for f in eval_features], dtype=torch.float)
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                    all_start_positions, all_end_positions, all_switches, all_span_mask)
        ...
        #num_train_optimization_steps = 200000
        all_input_ids = torch.tensor(
        ...
        all_span_mask = torch.tensor(
            [f.switch ==0  for f in train_features], dtype=torch.float)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
                ...
                input_ids, input_mask, segment_ids, start_positions, end_positions, switches, span_masks = batch
                start_loss, end_loss, switch_loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask,
                             start_positions=start_positions, end_positions=end_positions, switch_list=switches, span_mask=span_masks)
                if n_gpu > 1:
                    switch_loss = switch_loss.mean()  # mean() to average on multi-gpu.
                    start_loss = start_loss.mean()
                    end_loss = end_loss.mean()
                
                pp_start_loss += start_loss.data.cpu().numpy()
                pp_end_loss += end_loss.data.cpu().numpy()
                pp_switch_loss += switch_loss.data.cpu().numpy()
                loss = switch_loss + 0.2 * start_loss +  0.2 * end_loss
                #print(loss)
                pp_loss +=  loss.data.cpu().numpy()
                ...
                if step % 100 == 0:
                    logger.info("********* loss ************{}".format(print_loss))
                    #writer.add_scalar("printed_Loss_100_batch", print_loss, global_step)
                    print_loss = 0
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    #print(optimizer.optimizer)
                    #scheduler.step()
                    #writer.add_scalar("Learning_rate", optimizer.optimizer.param_groups[0]['lr'], global_step)
                    

                    writer.add_scalar("printed_start_Loss_every_update", pp_start_loss / 8, global_step)
                    writer.add_scalar("printed_end_Loss_every_update", pp_end_loss/ 8, global_step)
                    writer.add_scalar("printed_switch_Loss_every_update", pp_switch_loss/ 8, global_step)
                    writer.add_scalar("printed_loss_Loss_every_update", pp_loss / 8, global_step)
                    pp_loss = 0 
                    pp_start_loss = 0
                    pp_end_loss = 0
                    pp_switch_loss =0 
                    ...
                    if args.save_gran is not None:
                        ...
                        if  ( (global_step % save_chunk == 0) and (global_step // save_chunk >= save_start)):
                            ...
                            dev_switch_loss = 0 
                            dev_full_loss = 0 
                            for sstep, d_batch in enumerate(tqdm(eval_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
                                if n_gpu == 1:
                                    # multi-gpu does scattering it-self
                                    d_batch = tuple(t.to(device) for t in d_batch)
                                input_ids, input_mask, segment_ids, start_positions, end_positions, switches, span_masks = d_batch
                                d_start_loss, d_end_loss, d_switch_loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask,
                                            start_positions=start_positions, end_positions=end_positions, switch_list=switches, span_mask=span_masks)
                                if n_gpu > 1:
                                    d_switch_loss = d_switch_loss.mean()  # mean() to average on multi-gpu.
                                    d_start_loss = d_start_loss.mean()
                                    d_end_loss = d_end_loss.mean()
                                
                                dev_start_loss += d_start_loss.data.cpu().numpy()
                                dev_end_loss += d_end_loss.data.cpu().numpy()
                                dev_switch_loss += d_switch_loss.data.cpu().numpy()
                                d_loss = d_switch_loss + d_start_loss + d_end_loss
                                dev_full_loss += d_loss.data.cpu().numpy()
                            writer.add_scalar("printed_start_Loss_dev", dev_start_loss / sstep, global_step)
                            writer.add_scalar("printed_end_Loss_dev", dev_end_loss / sstep, global_step)
                            writer.add_scalar("printed_switch_Loss_dev", dev_switch_loss /sstep, global_step)
                            writer.add_scalar("printed_loss_Loss_dev", dev_full_loss /sstep, global_step)
                            logger.info("*********dev loss ************{}".format(dev_full_loss/sstep))
                            d_switch_loss = 0 
                            d_start_loss = 0
                            d_end_loss = 0
                            d_loss = 0
    ...
    #all_dir = ['bert_large_1114/15000steps/']
    #print(all_dir)
    #for dd in all_dir:
    #    args.output_dir = dd
    #    print(dd)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        output_prediction_file = os.path.join(
            args.output_dir, "predictions_test_final.json")
