                ...
                if is_training:
                    if version_2_with_negative:
                        ...
                        if is_impossible is False:
                            ...
                            switch = 1
                        #is_distint = qa["is_distant"]
                        #if is_distint is False:
                        #    distint = 0
                        #else:
                        #    distint = 1
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if example_index < 20 and not True:
                logger.info("*** Example ***")
...
def write_predictions_yes_no_no_empty_answer(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in tqdm(enumerate(all_examples)):
        ...
        print(features, len(features), no_masking)
        print(example)
        prelim_predictions = []
        ...
        print(all_predictions[example.qas_id])
    ...
    #    json.dump(all_nbest_json, writer, indent=2)
...
def write_predictions_yes_no_beam(all_examples, all_features, all_results, n_best_size,
                                  ...
                                  no_masking=False, output_selected_paras=True,
                                  quiet=True):
    ...
    for q_id, answers in q_id_to_answer_candidates.items():
        ...
        #print(min_no_ans_answers)
        #print(answers)
        #input()
        #if output_selected_paras is True:
        q_id_to_selected_para_lists[q_id] = list()
        for idx in np.argsort(no_answer_probs)[:10]:
            q_id_to_selected_para_lists[q_id].append(answers[idx]["para_titles"])
        #answers[min_no_ans_answers]["para_titles"]

        
    #print(q_id_to_selected_para_lists)
    try:
        with open(output_prediction_file, "w") as writer:
            writer.write(json.dumps(all_predictions, indent=4) + "\n")
        if output_selected_paras is True:
